...
- Responds with guidance when someone says `help`, `what do I do`, or `how do I use this`.
- Explains in plain English why something was flagged and what the suggested next step is.
...
- Creates a discussion thread on each report so Logistics can keep one check tidy.
...
## Help In Discord
Ollie now includes:
- `/olliehelp` for a quick usage guide
- `/scan` for a manual roster check
- `/olliestatus` for queue and live-check info
- `/member` for a member-specific explanation
- reply-based guidance when someone says `help`
