...
from ollie_bot.models import Issue
from ollie_bot.reporting import build_buttons_view, build_embed, build_report
...
class OllieBot(commands.Bot):
    ...
    async def _handle_reply_action(self, message: discord.Message) -> None:
        ...
        if any(token in lowered for token in ("sorted", "done", "reviewed", "handled")):
            ...
            await message.reply("Marked this report as reviewed.", mention_author=False)
            return

        if "snooze" in lowered or "later" in lowered:
            await self.db.update_issue_statuses(issue_ids, "snoozed")
            await message.reply("No problem. I've snoozed this report for now.", mention_author=False)
            return

        if "ignore" in lowered:
            await self.db.update_issue_statuses(issue_ids, "ignored")
            await message.reply("Okay. I'll ignore this one unless it shows up as new later.", mention_author=False)
            return

        if "needs vera" in lowered:
            await message.reply("Noted. I'll treat this as a Vera follow-up for Logistics.", mention_author=False)
            return

        if "draft dm" in lowered:
            draft = self._member_dm_draft()
            await message.reply(f"Here is the draft I would send:\n\n{draft}", mention_author=False)
            return
    ...
    async def _find_single_member_issue(self, issue_ids: list[str]) -> Issue | None:
        open_issues = await self.run_scan(persist=False)
    ...
    async def run_scan(self, persist: bool = True) -> list[Issue]:
        guild = self.get_guild(self.settings.guild_id)
        if guild is None:
