...
class OllieBot(commands.Bot):
    ...
    def _summary_reply(self, comparison: StatsComparison) -> str:
        if comparison.differences:
            return f"I posted the PvP roster member(s) who have not submitted stats yet."
        return "Everyone on the PvP roster has submitted stats."
...
def run_bot() -> None:
    ...
    async def _run_stats_command(interaction: discord.Interaction) -> None:
        ...
        if comparison.differences:
            await interaction.followup.send(
                "I've posted the PvP roster member(s) who have not submitted stats yet.",
                ephemeral=True,
            )
        else:
            await interaction.followup.send("Everyone on the PvP roster has submitted stats.", ephemeral=True)
