...
class OllieBot(commands.Bot):
    ...
    async def _reply_with_n3_count(self, message: discord.Message) -> None:
        roster = await self.sheet_client.fetch_roster()
        n3_entries = [entry for entry in roster if entry.worksheet_name.casefold() in {"n0va3 roster", "nova3 roster", "n3 roster"}]
    ...
    def _compare_group(self, guild: discord.Guild, roster: list[RosterEntry], group_key: str) -> dict[str, object] | None:
        roster_tab = ROSTER_TABS.get(group_key)
        role = self._resolve_group_role(guild, group_key)
        if roster_tab is None or role is None:
            return None

        roster_entries = [entry for entry in roster if entry.worksheet_name.casefold() == roster_tab.casefold()]
