...
from ollie_bot.models import Issue, RosterEntry, SavedNote
from ollie_bot.reporting import (
    build_buttons_view,
    build_embed,
    build_help_embed,
    build_help_menu_embed,
    build_help_topic_embed,
    build_report,
    build_status_embed,
)
...
class OllieBot(commands.Bot):
    def __init__(self, settings: Settings) -> None:
        ...
        self.note_task: asyncio.Task | None = None
        self.pending_suggestions: dict[tuple[int, int], str] = {}
        self.pending_help_topics: set[tuple[int, int]] = set()
    ...
    async def on_ready(self) -> None:
        ...
        if self.note_task is None:
            self.note_task = asyncio.create_task(self._note_review_scheduler())
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        handled_pending = await self._handle_pending_suggestion(message)
        if handled_pending:
            return
        handled_help = await self._handle_help_followup(message)
        if handled_help:
            return
        ...
        if "ollie help" in lowered or lowered.strip() == "help":
            self.pending_help_topics.add((message.channel.id, message.author.id))
            if message.author.id == self.settings.special_daddy_user_id:
                await message.reply("Here you go, daddy.", embed=build_help_menu_embed(), mention_author=False)
            else:
                await message.reply(embed=build_help_menu_embed(), mention_author=False)
            return
        ...
        note_handled = await self._handle_note_request(message)
        if note_handled:
            return
    ...
    async def _handle_natural_ollie_request(self, message: discord.Message) -> bool:
        ...
        if any(phrase in lowered for phrase in ("what do we know about", "show notes for", "what notes do we have on", "where did")):
            handled = await self._handle_note_lookup_request(message)
            if handled:
                return True

        if any(phrase in lowered for phrase in ("saved notes", "recent notes", "notes from this week")):
            await self._reply_with_recent_notes(message)
            return True
        ...
        if lowered.startswith("ollie ") and len(lowered.split()) <= 6:
            suggested = self._suggest_common_request(lowered)
            if suggested:
                self.pending_suggestions[(message.channel.id, message.author.id)] = suggested
                await message.reply(
                    f"Did you mean `{suggested}`? Just say `yes` or `no`, {self._address_for_user(message.author.id)}.",
                    mention_author=False,
                )
                return True
    ...
    async def _handle_pending_suggestion(self, message: discord.Message) -> bool:
        key = (message.channel.id, message.author.id)
        suggestion = self.pending_suggestions.get(key)
        if not suggestion:
            return False
        ...
        if lowered in {"no", "n"}:
            ...
            await message.reply(
                f"Okay, {self._address_for_user(message.author.id)}. Try asking a different way and I'll do my best with it.",
                mention_author=False,
            )
            return True

        self.pending_suggestions.pop(key, None)
        await self._run_suggested_request(message, suggestion)
        return True

    async def _handle_help_followup(self, message: discord.Message) -> bool:
        key = (message.channel.id, message.author.id)
        if key not in self.pending_help_topics:
            return False

        lowered = message.content.casefold().strip()
        if lowered in {"cancel", "stop", "never mind", "nevermind"}:
            self.pending_help_topics.discard(key)
            await message.reply("No problem. I've closed the help menu for now.", mention_author=False)
            return True

        if lowered == "everything":
            self.pending_help_topics.discard(key)
            await message.reply(embed=build_help_embed(), mention_author=False)
            return True

        help_topics = {"scan", "review", "vera", "compare", "flags", "notes", "reminders", "commands"}
        if lowered in help_topics:
            self.pending_help_topics.discard(key)
            await message.reply(embed=build_help_topic_embed(lowered), mention_author=False)
            return True

        return False

    async def _handle_note_request(self, message: discord.Message) -> bool:
        lowered = message.content.casefold().strip()
        note_phrases = ("ollie make a note of this", "ollie remember this", "ollie keep this on file", "ollie save this note")
        if not any(phrase in lowered for phrase in note_phrases):
            return False

        if not message.reference or not message.reference.message_id:
            await message.reply(
                f"Reply to the message you want me to save and say `ollie make a note of this`, {self._address_for_user(message.author.id)}.",
                mention_author=False,
            )
            return True
        ...
        existing = await self.db.find_note_by_source_message(source.id)
        if existing is not None:
            await message.reply(
                f"I already have that saved as note `{existing.note_id}`, {self._address_for_user(message.author.id)}.",
                mention_author=False,
            )
            return True

        player_name = await self._guess_note_player_name(source.content)
        note_id = await self.db.create_note(
            source_message_id=source.id,
            source_author_id=source.author.id,
            requested_by_id=message.author.id,
            channel_id=message.channel.id,
            content=source.content or "[No text content]",
            source_author_name=source.author.display_name,
            requested_by_name=message.author.display_name,
            player_name=player_name,
        )
        if player_name:
            reply = f"I've saved that as note `{note_id}` and it looks like it may relate to `{player_name}`. I'll check back in 72 hours to see whether you still want it kept."
        else:
            reply = f"I've saved that as note `{note_id}`. I'll check back in 72 hours to see whether you still want it kept."
        await message.reply(reply, mention_author=False)
        return True

    def _suggest_common_request(self, lowered: str) -> str | None:
        suggestions = [
            "ollie scan",
            "ollie who needs review",
            "ollie show vera cases",
            ...
            "ollie name mismatches",
            "ollie keepshare checks",
            "ollie summary",
        ]
        matches = get_close_matches(lowered, suggestions, n=1, cutoff=0.55)
        return matches[0] if matches else None

    async def _run_suggested_request(self, message: discord.Message, suggestion: str) -> None:
        lowered = suggestion.casefold()
        if lowered == "ollie scan":
            issues = await self.publish_scan_report(force=True)
            reply = (
                f"I've run a fresh scan and posted {len(issues)} issue(s) in Logistic Council for you."
                if issues
                else "I've run a fresh scan and there is nothing new to report right now."
            )
            await message.reply(reply, mention_author=False)
            return
        if lowered == "ollie who needs review":
            await self._reply_with_review_summary(message)
            return
        if lowered == "ollie show vera cases":
            await self._reply_with_vera_cases(message)
            return
        if lowered == "ollie draft reminders":
            await self._reply_with_reminder_targets(message)
            return
        if lowered == "ollie compare pvp to the roster":
            await self._reply_with_group_comparison(message, "pvp")
            return
        if lowered == "ollie compare n0va to the roster":
            await self._reply_with_group_comparison(message, "n0va")
            return
        if lowered == "ollie compare n3 to the roster":
            await self._reply_with_group_comparison(message, "n3")
            return
        if lowered == "ollie name mismatches":
            await self._reply_with_name_mismatches(message)
            return
        if lowered == "ollie keepshare checks":
            await self._reply_with_keepshare_checks(message)
            return
        if lowered == "ollie summary":
            await self._reply_with_all_group_overview(message)
            return

    async def _handle_note_lookup_request(self, message: discord.Message) -> bool:
        lowered = message.content.casefold()
        query = message.content
        prefixes = [
            "ollie what do we know about",
            "ollie show notes for",
            "ollie what notes do we have on",
            "ollie where did",
        ]
        for prefix in prefixes:
            if lowered.startswith(prefix):
                ...
                break
        if not query or query.casefold().startswith("ollie "):
            return False
        notes = await self.db.search_notes(query)
        if not notes:
            await message.reply(f"I couldn't find any saved notes for `{query}` right now.", mention_author=False)
            return True
        lines = [f"Here is what I have saved for `{query}`:"]
        for note in notes[:5]:
            owner = note.player_name or note.source_author_name
            lines.append(f"- Note `{note.note_id}` on {owner}: {note.content[:180]}")
        await message.reply("\n".join(lines), mention_author=False)
        return True

    async def _reply_with_recent_notes(self, message: discord.Message) -> None:
        notes = await self.db.recent_notes(days=7)
        if not notes:
            await message.reply("I don't have any saved notes from the last 7 days.", mention_author=False)
            return
        lines = ["Recent saved notes:"]
        for note in notes[:8]:
            owner = note.player_name or note.source_author_name
            lines.append(f"- `{note.note_id}` for {owner}: {note.content[:140]}")
        if len(notes) > 8:
            lines.append(f"...and {len(notes) - 8} more.")
        await message.reply("\n".join(lines), mention_author=False)

    async def _guess_note_player_name(self, content: str) -> str | None:
        roster = await self.sheet_client.fetch_roster()
        lowered = content.casefold()
        for entry in roster:
            roster_name = first_non_empty((entry.ign, entry.display_name))
            if roster_name and roster_name.casefold() in lowered:
                return roster_name
        return None
    ...
    async def _reply_with_group_comparison(self, message: discord.Message, group: str) -> None:
        guild = self.get_guild(self.settings.guild_id)
        if guild is None:
            await message.reply("I couldn't reach the server for that comparison, lovely.", mention_author=False)
            return
        ...
        result = self._compare_group(guild, roster, group)
        if result is None:
            await message.reply(f"I couldn't build a comparison for {group} cleanly.", mention_author=False)
            return
        lines = [
            f"{group.upper()} comparison:",
            f"- Discord role count: {result['role_count']}",
    ...
    async def _handle_reply_action(self, message: discord.Message) -> None:
        ...
        note = await self.db.find_note_by_reminder_message(referenced_id)
        if note is not None:
            await self._handle_note_followup_reply(message, note)
            return
    ...
    async def _handle_note_followup_reply(self, message: discord.Message, note: SavedNote) -> None:
        lowered = message.content.casefold()
        if any(token in lowered for token in ("keep", "yes", "keep it", "save it", "permanent")):
            await self.db.update_note_status(note.note_id, "kept", note.player_name)
            await message.reply(f"I've kept note `{note.note_id}` on file, {self._address_for_user(message.author.id)}.", mention_author=False)
            return
        if any(token in lowered for token in ("discard", "delete", "remove", "clear", "no")):
            await self.db.update_note_status(note.note_id, "discarded", note.player_name)
            await message.reply(f"Okay, {self._address_for_user(message.author.id)}. I've cleared note `{note.note_id}`.", mention_author=False)
            return
        if "attach" in lowered or "player" in lowered:
            player_name = await self._guess_note_player_name(note.content)
            await self.db.update_note_status(note.note_id, "kept", player_name or note.player_name)
            owner = player_name or note.player_name or "that player"
            await message.reply(f"I've attached note `{note.note_id}` to `{owner}` and kept it on file.", mention_author=False)
            return
        await message.reply(
            "If you want me to act on this note, reply with `keep`, `discard`, or `attach to player`.",
            mention_author=False,
        )
    ...
    async def _note_review_scheduler(self) -> None:
        await self.wait_until_ready()
        while not self.is_closed():
            try:
                await self._send_due_note_reviews()
            except Exception:
                LOGGER.exception("Note review scheduler failed")
            await asyncio.sleep(600)

    async def _send_due_note_reviews(self) -> None:
        due_notes = await self.db.get_due_notes()
        if not due_notes:
            return
        guild = self.get_guild(self.settings.guild_id)
        if guild is None:
        ...
        for note in due_notes:
            requester = guild.get_member(note.requested_by_id)
            channel = guild.get_channel(note.channel_id)
            if requester is None or not isinstance(channel, discord.TextChannel):
                continue
            message = await channel.send(
                f"{requester.mention} you asked me to keep note `{note.note_id}` from 72 hours ago.\n"
                f"Source: {note.source_author_name}\n"
                f"Note: {note.content[:250]}\n\n"
                "Reply to this with `keep`, `discard`, or `attach to player` and I'll sort it."
            )
            await self.db.attach_note_reminder(note.note_id, message.id)
...
def run_bot() -> None:
    ...
    @bot.tree.command(name="notes", description="Search saved logistics notes.", guild=discord.Object(id=settings.guild_id))
    @app_commands.describe(query="A player name, team, or keyword")
    async def notes(interaction: discord.Interaction, query: str) -> None:
        await interaction.response.defer(ephemeral=True, thinking=True)
        results = await bot.db.search_notes(query)
        if not results:
            await interaction.followup.send(f"I couldn't find any saved notes for `{query}`.", ephemeral=True)
            return
        embed = discord.Embed(
            title=f"Ollie Notes: {query}",
            description="Here are the closest saved notes I found.",
            color=discord.Color.from_str("#8C52FF"),
        )
        for note in results[:5]:
            owner = note.player_name or note.source_author_name
            embed.add_field(
                name=f"Note {note.note_id} · {owner}",
                value=f"{note.content[:220]}\nStatus: {note.status}",
                inline=False,
            )
        await interaction.followup.send(embed=embed, ephemeral=True)
