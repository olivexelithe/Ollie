...
from ollie_bot.utils import first_non_empty, names_loosely_match, normalize_name, stable_issue_id
...
class RosterScanner:
    ...
    def scan(
        ...
        *,
        include_guest_pass_reviews: bool = True,
    ) -> list[Issue]:
        ...
        for member in guild.members:
            ...
            if not matched_entry:
                ...
                elif has_guest_pass and include_guest_pass_reviews:
                    issues.append(
                        Issue(
                            issue_id=stable_issue_id("guest-review", str(member.id)),
                            kind="guest_pass_review",
                            severity="low",
                            confidence="medium",
                            title="Guest pass member for review",
                            summary=f"{member.display_name} is currently in guest pass and does not appear on the roster.",
                            member_id=member.id,
                            member_name=member.display_name,
                            roster_name=None,
                            recommended_action="Review whether this guest pass member should stay in guest pass, return to the roster, or be removed.",
                        )
                    )
                continue
            ...
            if not names_loosely_match(roster_name, nickname):
                issues.append(
                    Issue(
    ...
    def _find_matching_entries(
        ...
        for entry in roster:
            ...
            if not normalized_roster_name:
                continue
            if any(
                normalized_roster_name in candidate
                or names_loosely_match(normalized_roster_name, candidate)
                for candidate in candidates
            ):
